...
install_greenlight(){
  ...
  SECRET_KEY_BASE=$(docker run --rm rezaejteh/greenlight:latest bundle exec rake secret)

  if [ ! -f ~/greenlight/.env ]; then
    docker run --rm rezaejteh/greenlight:latest cat ./sample.env > ~/greenlight/.env
  fi
  ...
  if [ ! -f /etc/bigbluebutton/nginx/greenlight.nginx ]; then
    docker run --rm rezaejteh/greenlight:latest cat ./greenlight.nginx | tee /etc/bigbluebutton/nginx/greenlight.nginx
...
HERE
  ...
  if [ ! -f ~/greenlight/docker-compose.yml ]; then
    docker run --rm rezaejteh/greenlight:latest cat ./docker-compose.yml > ~/greenlight/docker-compose.yml
  fi
